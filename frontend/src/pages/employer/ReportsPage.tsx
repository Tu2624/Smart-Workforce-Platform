    ...
    <DashboardLayout>
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8 pb-12">
        ...
        <motion.div variants={itemVariants}>
          ...
          <p className="text-slate-400 text-sm mt-1">Tổng hợp dữ liệu hoạt động trong tháng hiện tại</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <motion.div variants={itemVariants}>
            <Card className="p-5">
              <h2 className="text-sm font-black text-white mb-4">Chi phí lương 6 tháng gần nhất</h2>
              {payroll.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={payroll} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                    ...
                    />
                    <Bar dataKey="total_amount" fill="#6366f1" radius={[4, 4, 0, 0]} name="Chi phí" />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </Card>
          ...
          <motion.div variants={itemVariants}>
            <Card className="p-5">
              <h2 className="text-sm font-black text-white mb-4">Ca làm 30 ngày qua</h2>
              {shifts.length === 0 ? (
                <p className="text-slate-500 text-sm text-center py-10">Chưa có dữ liệu</p>
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={shifts} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                    ...
                    <Bar dataKey="completed" fill="#10b981" radius={[3, 3, 0, 0]} name="Hoàn thành" stackId="a" />
                    <Bar dataKey="upcoming"  fill="#6366f1" radius={[3, 3, 0, 0]} name="Sắp tới"    stackId="a" />
        ...
        <motion.div variants={itemVariants}>
          <Card className="p-5">
            <h2 className="text-sm font-black text-white mb-4">Hiệu suất nhân viên</h2>
            {perf.length === 0 ? (
              <p className="text-slate-500 text-sm text-center py-10">Chưa có dữ liệu</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {perf.map((e, i) => (
                      <tr key={e.id} className={`border-b border-slate-800/50 ${i % 2 === 0 ? '' : 'bg-slate-800/20'}`}>
                        <td className="py-2.5 px-3 font-semibold text-white whitespace-nowrap">{e.full_name}</td>
                        <td className="py-2.5 px-3 text-slate-300">{e.total_shifts}</td>
                        <td className="py-2.5 px-3 text-emerald-400">{e.on_time_count}</td>
                        <td className="py-2.5 px-3 text-yellow-400">{e.late_count}</td>
                        <td className="py-2.5 px-3 text-rose-400">{e.absent_count}</td>
                        <td className="py-2.5 px-3 text-slate-300">{e.total_hours.toFixed(1)}h</td>
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 bg-slate-700 rounded-full h-1.5 min-w-16">
                              <div className="bg-emerald-400 h-1.5 rounded-full" style={{ width: `${e.on_time_rate}%` }} />
                            </div>
                            <span className="text-slate-300 text-xs whitespace-nowrap">{e.on_time_rate}%</span>
                          </div>
                        </td>
                        <td className="py-2.5 px-3">
                          <span className={`font-black ${reputationColor(e.reputation_score)}`}>
                            {e.reputation_score}
                          </span>
