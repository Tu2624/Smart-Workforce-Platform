...
const navItems = [
  ...
  { to: '/student/schedule', label: 'Lịch làm' },
  { to: '/student/shifts', label: 'Tìm ca' },
  { to: '/student/attendance', label: 'Chấm công' },
  { to: '/student/payroll', label: 'Lương' },
        ...
        <nav className="flex-1 p-3 space-y-1">
          {navItems.map((item) => (
            <NavLink key={item.to} to={item.to} end={item.end}
              className={({ isActive }) =>
                `flex items-center px-3 py-2 rounded-lg text-sm transition-colors ${isActive ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
              {item.label}
